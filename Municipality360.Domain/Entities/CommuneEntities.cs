...
public class Reclamation : BaseEntity
{
    ...
    // ── التصنيف الآلي (ML.NET) ───────────────────────────────────

    /// <summary>الفئة المقترحة بواسطة الذكاء الاصطناعي (قبل مراجعة الوكيل)</summary>
    public string? AISuggestedCategory { get; set; }

    /// <summary>درجة ثقة النموذج في التصنيف [0.0 → 1.0]</summary>
    public float? AIConfidenceScore { get; set; }

    /// <summary>الأولوية المقترحة آلياً (Basse/Moyenne/Haute/Critique)</summary>
    public string? AISuggestedPriorite { get; set; }

    /// <summary>درجة الخطورة كما قدّرها النموذج [0 → 10]</summary>
    public float? AISeverityScore { get; set; }

    /// <summary>هل تم تطبيق التصنيف الآلي بنجاح؟</summary>
    public bool AIClassificationApplied { get; set; } = false;

    /// <summary>تاريخ تشغيل خطوط الذكاء الاصطناعي</summary>
    public DateTime? AIProcessedAt { get; set; }

    // ── الرد الآلي (Generative AI) ────────────────────────────────

    /// <summary>نص الرد الرسمي الآلي المُولَّد للمواطن (عربي فصيح)</summary>
    public string? AutoGeneratedResponse { get; set; }

    /// <summary>هل أُرسل الرد الآلي للمواطن؟</summary>
    public bool AutoResponseSent { get; set; } = false;

    /// <summary>تاريخ إرسال الرد الآلي</summary>
    public DateTime? AutoResponseSentAt { get; set; }

    /// <summary>
    /// طوابع الكلمات المفتاحية المستخرجة من وصف الشكوى
    /// مخزّنة كـ JSON: ["طريق","حفرة","إنارة"]
    /// </summary>
...
// ════════════════════════════════════════════════════════════════
//  Domain/Entities/AIModels.cs  — كيانات مساعدة للذكاء الاصطناعي
// ════════════════════════════════════════════════════════════════

/// <summary>
/// نموذج بيانات الإدخال لـ ML.NET (لا يرث من BaseEntity — ليس جدول DB)
/// يُستخدم فقط داخل PredictionEngine
/// </summary>
public sealed class ReclamationMLInput
{
    /// <summary>نص الشكوى (الوصف + الموضوع مدمجان)</summary>
    public string Text { get; set; } = string.Empty;
}

/// <summary>نموذج الإخراج من ML.NET</summary>
public sealed class ReclamationMLOutput
{
    /// <summary>الفئة المتوقعة (تطابق قيم CategorieReclamation.Code)</summary>
    public string PredictedLabel { get; set; } = string.Empty;

    /// <summary>مصفوفة الاحتماليات لكل فئة</summary>
    public float[] Score { get; set; } = [];

    /// <summary>أعلى احتمالية — تُحسب بعد التنبؤ</summary>
    public float MaxScore => Score.Length > 0 ? Score.Max() : 0f;
}
