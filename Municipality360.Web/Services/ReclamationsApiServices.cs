...
public class ReclamationApiService : IReclamationApiService
{
    ...
    {
        var r = await _api.PatchAsync<AssignerReclamationDto, object>($"api/reclamations/{id}/assigner", dto);
        return r != null;
    }
    public async Task<bool> ChangerStatutAsync(int id, ChangerStatutReclamationDto dto)
    {
        var r = await _api.PatchAsync<ChangerStatutReclamationDto, object>($"api/reclamations/{id}/statut", dto);
        return r != null;
    }
}
...
public class CitoyenApiService : ICitoyenApiService
{
    ...
    public async Task<bool> CINExistsAsync(string cin, int? excludeId = null)
    {
        try
        {
            var q = $"api/citoyens/cin-exists/{Uri.EscapeDataString(cin)}";
...
public class TypeReclamationApiService : ITypeReclamationApiService
{
    ...
    public async Task<TypeReclamationDto?> GetByIdAsync(int id) =>
        await _api.GetAsync<TypeReclamationDto>($"api/types-reclamation/{id}");
...
public class CategorieReclamationApiService : ICategorieReclamationApiService
{
    ...
    public async Task<List<CategorieReclamationDto>?> GetFlatAsync() =>
        await _api.GetAsync<List<CategorieReclamationDto>>("api/categories-reclamation/flat");
    public async Task<CategorieReclamationDto?> GetByIdAsync(int id) =>
        await _api.GetAsync<CategorieReclamationDto>($"api/categories-reclamation/{id}");
